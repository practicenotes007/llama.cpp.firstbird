...
- [x] 搭建基准测试框架 (`bench/` 目录: run_bench.sh, collect_sys.sh, run_stream.sh, compare.sh, plot_results.py, run_all.sh, test_kernels.c)
- [x] Makefile 集成: `make test`, `make bench`, `make bench-ddr`, `make bench-compare`, `make bench-plot`, `make bench-all`
- [x] FP16 ↔ FP32 转换正确性测试
  - [x] 遍历全部 65536 个 FP16 bit pattern，验证 `ggml_fp16_to_fp32` 与 IEEE 754 参考实现一致
  - [x] 验证 `table_f32_f16` 查表结果与逐次计算结果一致
- [x] GELU / SiLU 查表正确性测试
  - [x] 对 `table_gelu_f16` / `table_silu_f16` 抽样验证 (最大相对误差 < 0.05)
- [x] 量化 / 反量化正确性测试
  - [x] Q4_0 round-trip 测试 (各种数据分布, RMSE < 0.5)
  - [x] 向量点积 (dot product) SIMD vs 标量参考值误差测试 (< 2%)
- [ ] SIMD 辅助函数正确性测试
...
| 2026-06-03 | 搭建完整基准测试框架: bench/run_bench.sh, run_all.sh, test_kernels.c, plot_results.py, compare.sh, collect_sys.sh, run_stream.sh + Makefile 集成 |
| 2026-06-03 | DDR 带宽校准附录写入 OPTIMIZATION_NOTE |
| 2026-06-03 | NPU 带宽误区修正 (NPU 共享 DDR4 总线, 不减少权重搬运) |
